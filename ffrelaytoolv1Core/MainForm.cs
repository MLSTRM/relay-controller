        ...
        public void handleOutboundMessages(List<OutboundMessage> messages)
        {
            ...
            if (messages.Count > 0)
            {
                foreach (OutboundMessage m in messages)
                {
                    if(m.time < 0)
                    {
                        consumptionLog += "Ignored due to negative time value on split";
                        continue;
                    }
            ...
            Debug.WriteLine(consumptionLog);
            Console.WriteLine(consumptionLog);
            ...
            else
            {
                if (meta.features.infoCyclingPattern.Length > 0)
                {
                    ...
                    if (toCycle)
                    {
                        ...
                        // TODO: should be index of graph tab.
